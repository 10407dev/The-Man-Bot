...
async function changeRoles(i: MessageComponentInteraction) {
	const roles = "values" in i ? (i.values as string[]) : [i.customId];
	let message = { content: "Changed roles:\n", ephemeral: true };
	const member = await i.guild!.members.fetch(i.user);
	roles.forEach((role) => {
		if (member.roles.cache.has(role)) member.roles.remove(role).then(() => (message.content += `**-** <@&${role}>\n`));
		else member.roles.add(role).then(() => (message.content += `**+** <@&${role}>\n`));
	});
