...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("menu")
		.setDescription("Send or update info menu")
		.setDMPermission(false)
		.addSubcommand((subcommand) => subcommand.setName("send").setDescription("Send info menu"))
		.addSubcommand((subcommand) => subcommand.setName("update").setDescription("Update existing info menu")),
	execute: async (i: ChatInputCommandInteraction) => {
		const infoChannel = await i.guild!.channels.fetch(process.env.INFO_CHANNEL_ID!);
		...
		if (i.options.getSubcommand(false) === "send") {
			(infoChannel as TextBasedChannel).send(menu.mainMessage);
		} else {
			(infoChannel as TextBasedChannel).messages.fetchPinned().then((m) => {
				const msg = m.first() as Message;
				msg.edit(menu.mainMessage);
			});
		}
	},
};
...
const menu = {
	mainMessage: {
		embeds: [
			new EmbedBuilder()
				.setImage("https://media.discordapp.net/attachments/1070617876601520178/1074560345919725608/EMPIRE_OF_MAN_Banner.png")
				.setColor("#2B2D31"),
			new EmbedBuilder()
				.setImage("https://media.discordapp.net/attachments/1070617876601520178/1074559129965842492/Fictional_Men_Banner.png")
				.setColor("#2B2D31")
...
async function menuActions(i: MessageComponentInteraction) {
	if (i.customId.endsWith("Role")) {
		let message = { content: "Changed roles:\n", ephemeral: true };
		const inter = i as StringSelectMenuInteraction;
		const member = await inter.guild!.members.fetch(i.user);
		inter.values.forEach((role) => {
		...
		i.update({ components: menu.messages.roles.components }).then(() => i.followUp(message));
	}
}
